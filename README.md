...
# [OK] Indexed N chunks into Qdrant Cloud.
```
...
# Windows PowerShell:
# Get-ChildItem data/*.pdf | ForEach-Object { python ingest.py $_.FullName "2025-01-01" }
```
...
`.github/workflows/rag_eval.yml` triggers on pull requests to `main` affecting `app.py`, `ingest.py`, `config/**`, `tests/**`, `requirements.txt`. It:

1. Validates the 50-item schema
2. Runs `pytest tests/test_ragas_eval.py`
...
- **Chunk geometry:** 600-word window, 100-word overlap (≈500 stride), ≥120 chars, noise filter (`advertisement`, `subscribe`, `page `, `epaper`), sentence-boundary snap at ≥60% coverage.
- **Collection:** `wealth_archive`, 384-dim cosine, `HnswConfigDiff(m=16, ef_construct=128, full_scan_threshold=10_000)`, payload indexes on `edition_date` (keyword), `page_number` (integer), `source` (keyword).
- **IDs:** `generate_point_id` = MD5(`"{date}|p{page}|c{idx}|{prefix[:50]}"`), `generate_chunk_id` = `chk_YYYY_MM_DD_p{page}_{idx:03d}` (idempotent).
- **Fusion:** `Score = (1/(60+r_d)+1/(60+r_s)) * (1+0.35*exp(-Δt/365))`.
- **Refusal:** `θ=0.25`, `min_relevant_chunks=1` — any of empty, top-1 < θ, or insufficient relevant triggers deterministic refusal (no LLM call).
- **Rate limiting:** `GeminiRateLimiter(max_rpm=14)` (interval ~4.3s), 429 backoff 5s/10s/20s, `with_qdrant_retry` exponential backoff 0.5s/1s/2s with ±25% jitter on `{429,502,503,504}`.
- **Tracing:** `QueryTrace` dataclass + `timer()` context manager (perf_counter) + `emit()` JSON log.
- **Prompts:** All templates in `config/prompts.yaml` (`prompt_version: 1.0.0`) — validated via `string.Formatter.parse()` for `{context}` and `{query}`.
